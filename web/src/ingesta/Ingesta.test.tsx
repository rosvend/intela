...
describe("pantalla de ingesta (integracion con App)", () => {
  ...
  it("un 4xx con un cuerpo que no es JSON no pinta ese cuerpo como mensaje del backend", async () => {
    ...
    // El 409 tiene DOS ramas -el duplicado, donde la entrega ya esta, y la
    // evidencia corrupta, donde no- y el TITULO solo ve el status, asi que no
    // puede elegir rama. Aqui ademas el cuerpo que llega es la pagina del
    // proxy, no el mensaje del backend. Por eso el titulo no puede afirmar ni
    // el registro ni el no-registro, y solo nombra el conflicto; el mensaje de
    // debajo dice cual de las dos es cuando llega legible.
