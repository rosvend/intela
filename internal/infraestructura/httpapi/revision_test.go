...
func servidorConCola(t *testing.T, cola ColaRevision) http.Handler {
	...
	return Nueva(Casos{Auth: auth, Cola: cola}, Opciones{}).Router()
}
...
func TestListarColaRevisionExigeAdministrador(t *testing.T) {
	...
	h := Nueva(Casos{Auth: auth, Cola: &colaFalsa{}}, Opciones{}).Router()
...
func TestListarColaRevisionSinSesionEs401(t *testing.T) {
	h := Nueva(Casos{Auth: &autenticacionFalsa{}, Cola: &colaFalsa{}}, Opciones{}).Router()
