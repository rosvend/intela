...
// ReporteLiquidaciones es lo que la capa HTTP necesita del nucleo para el
// panel por obra y el export (#43). Se declara en el consumidor, igual que
// [ConsultaLiquidaciones]: el adaptador depende de dos metodos, no del
// struct que los implementa.
type ReporteLiquidaciones interface {
...
func (a *API) consultarLiquidaciones(w http.ResponseWriter, r *http.Request) {
	if a.reporte == nil {
		escribirError(w, http.StatusServiceUnavailable,
			"el reporte de liquidacion no esta configurado en esta instalacion")
		return
	}
	...
	liq, err := a.reporte.Consultar(r.Context(), actor, r.URL.Query().Get("periodo"))
...
func (a *API) exportarLiquidaciones(w http.ResponseWriter, r *http.Request) {
	...
	q := r.URL.Query()
	archivo, err := a.reporte.Exportar(r.Context(), actor, q.Get("periodo"), q.Get("formato"))
	if !escribirErrorLiquidacion(w, err) {
		return
	}
