...
func servidorConReporte(t *testing.T, auth Autenticacion, liq ReporteLiquidaciones) http.Handler {
	t.Helper()
	return Nueva(Casos{Auth: auth, Reporte: liq}, Opciones{}).Router()
}
...
func TestConsultarLiquidacionesDevuelveElPanel(t *testing.T) {
	...
	rec := pedir(t, servidorConReporte(t, auth, fake), http.MethodGet, "/mis-liquidaciones/obras?periodo=2026-01", "", "tok")
...
func TestExportarLiquidacionPDFCabecerasYCuerpo(t *testing.T) {
	...
	rec := pedir(t, servidorConReporte(t, auth, fake), http.MethodGet,
...
func TestMisLiquidacionesSinSesionEs401(t *testing.T) {
	h := servidorConReporte(t, &autenticacionFalsa{}, &liquidacionesFalsa{})
	for _, ruta := range []string{"/mis-liquidaciones/obras", "/mis-liquidaciones/export?formato=pdf"} {
...
func TestMisLiquidacionesConOtroRolEs403(t *testing.T) {
	...
	h := servidorConReporte(t, auth, &liquidacionesFalsa{})

	rec := pedir(t, h, http.MethodGet, "/mis-liquidaciones/obras", "", "tok")
