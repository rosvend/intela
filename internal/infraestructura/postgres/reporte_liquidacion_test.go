...
func TestDeTitularDevuelveNetoYTotalesDelProceso(t *testing.T) {
	...
	filas, err := s.FilasDeTitular(t.Context(), titularAna, "2026-01")
	if err != nil {
...
func TestDeTitularRespetaElPeriodo(t *testing.T) {
	...
	filas, err := s.FilasDeTitular(t.Context(), titularAna, "2025-06")
	if err != nil {
		t.Fatalf("DeTitular: %v", err)
	}
	if len(filas) != 0 {
...
func TestDeTitularSinFiltroTraeTodosLosPeriodos(t *testing.T) {
	...
	filas, err := s.FilasDeTitular(t.Context(), titularAna, "")
	if err != nil {
		t.Fatalf("DeTitular: %v", err)
	}
	if len(filas) != 2 {
...
func TestDeTitularNoMezclaTitulares(t *testing.T) {
	...
	filas, err := s.FilasDeTitular(t.Context(), titularBeto, "2026-01")
	if err != nil {
		t.Fatalf("DeTitular: %v", err)
	}
	if len(filas) != 1 {
...
func TestDeTitularNetosProcesoPermitenReconciliarDeducciones(t *testing.T) {
	...
	ana, err := s.FilasDeTitular(t.Context(), titularAna, "2026-01")
	if err != nil {
		t.Fatalf("ana: %v", err)
	}
	beto, err := s.FilasDeTitular(t.Context(), titularBeto, "2026-01")
	if err != nil {
		t.Fatalf("beto: %v", err)
	}
...
func sembrarCorridaReporte(t *testing.T, s *Store) {
	t.Helper()
