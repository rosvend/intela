...
// BuscarTitulares resuelve los tres filtros y la paginacion en una consulta.
//
// # Los filtros
//
// Nombre, IPI y persona natural se neutralizan dentro del WHERE con su valor
// cero -cadena vacia, y NULL para el booleano-, que es la misma forma del
...
func (s *Store) BuscarTitulares(ctx context.Context, f aplicacion.FiltroTitulares) ([]afiliacion.Titular, error) {
	...
	sql := `SELECT ` + columnasTitular + `
	        ...
	        ORDER BY id`
	args := []any{f.Nombre, patronContiene(f.Nombre), f.IPI, f.PersonaNatural}
