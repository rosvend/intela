...
const columnasUso = `id, reporte_id, fuente, titulo, ids_fuente, COALESCE(obra_id, ''),
	escalon, evidencia, oni, modalidad, tipo_obra, canal_id,
...
func escanearUso(fila pgx.Row) (aplicacion.UsoPersistido, error) {
	...
	err := fila.Scan(
		...
		&u.Escalon, &u.Evidencia, &u.ONI, &modalidad, &u.TipoObra, &u.CanalID,
...
func insertarUso(ctx context.Context, tx pgx.Tx, u aplicacion.UsoPersistido) error {
	_, err := tx.Exec(ctx,
		`INSERT INTO usos (
		   ...
		   oni, modalidad, tipo_obra, canal_id,
		   duracion_min, emisiones, rating, taquilla, espectadores, exhibiciones,
		   vistas, minutos_vistos, pb)
		...
		u.ONI, string(u.Modalidad), u.TipoObra, u.CanalID,
