...
func TestProrratearRedondeoCierraPorLinea(t *testing.T) {
	// Split real: 100/3 no es exacto en centavos. La identidad de la linea manda.
	l := ProrratearLinea(dec("2500.01"), dec("1000"), dec("500"), dec("1000"), dec("7500"))
	suma := l.Neto.Add(l.Admin).Add(l.Social).Add(l.Reserva)
	if !suma.Equal(l.Bruto) {
		t.Fatalf("bruto %s != neto+deducciones %s (Bruto - Σ Deducciones no cuadra con Neto)", l.Bruto, suma)
	}
	if !l.Neto.Equal(l.Bruto.Sub(l.Admin).Sub(l.Social).Sub(l.Reserva)) {
		t.Fatalf("neto mostrado %s != Bruto - Σ Deducciones", l.Neto)
	}
}
