module github.com/rosvend/intela
...
require (
	...
	// Fijada a proposito, por lo mismo que aws-lambda-go: v2.11.0 declara
	// `go 1.25`, y con eso `go mod tidy` sube la directiva `go` de este modulo.
	// Eso rompe el Dockerfile (golang:1.24-bookworm) y cambia la version de Go
...
require (
	...
	// version de Go que instala CI, que la lee de aqui.
	//
	// No se compila nunca: entra por el test de una dependencia
	// (docker/docker/client.test -> otelhttp -> otlp -> grpc-gateway), que
	// tidy resuelve aunque no lo construyamos. v2.27.1 declara `go 1.23.0`.
	//
	// Subir la version de Go del proyecto es una decision de stack, con su ADR;
	// no algo que se cuele por el grafo de un adaptador.
